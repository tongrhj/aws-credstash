...
function compareParams(actual, expected) {
  ...
  if (expected.ExpressionAttributeNames) {
    actual.ExpressionAttributeNames.should.eql(
  ...
  if (expected.ExpressionAttributeValues) {
    actual.ExpressionAttributeValues.should.eql(
...
describe("dynmaodDb", () => {
  ...
  describe("#getAllSecretsAndVersions", () => {
    it("should properly page through many results", () => {
      ...
      return dynamo
        .getAllSecretsAndVersions({ limit: 10 })
        .then((res) => res.Items)
        .then((secrets) => {
  ...
  describe("#getLatestVersion", () => {
    it("should only get one item back", () => {
      ...
      dynamo = new DynamoDb(TableName, { region: "us-east-1" });
      return dynamo.getLatestVersion("").then((res) => {
        expect(res).to.exist;
  ...
  describe("#getByVersion", () => {
    it("should only get one item back", () => {
      ...
      dynamo = new DynamoDb(TableName, { region: "us-east-1" });
      return dynamo.getByVersion(name, version).then((res) => {
        expect(res).to.exist;
  ...
  describe("#deleteSecret", () => {
    it("should delete the secret by name and version", () => {
      ...
      dynamo = new DynamoDb(TableName, { region: "us-east-1" });
      return dynamo.deleteSecret(name, version).then((secret) => {
  ...
  describe("#createTable", () => {
    it("should create the table with the HASH as name and RANGE as version", function () {
      ...
      AWS.mock("DynamoDB", "describeTable", (params, cb) =>
        cb({ code: "ResourceNotFoundException" })
      );
      AWS.mock("DynamoDB", "createTable", (params, cb) => {
        ...
        const name = params.AttributeDefinitions.find(
          (next) => next.AttributeName == "name"
        );
        ...
        const version = params.AttributeDefinitions.find(
          (next) => next.AttributeName == "version"
        );
