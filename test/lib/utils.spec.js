...
describe("utils", () => {
  ...
  describe("#sortSecrets", () => {
    ...
    it("should sort by version in descending order", () => {
      const array = Array.from({ length: 10 }, (k, i) => ({
        name: "same",
        version: `0${i}`,
      }));
      fisherYates(array);
  ...
  describe("#asPromise", () => {
    ...
    it("should insert a callback", () => {
      ...
      return utils.asPromise({}, fn).then((res) => res.should.equal("Success"));
    });
  ...
  describe("#mapPromise", () => {
    it("calls the promises in order", function () {
      ...
      return utils
        .mapPromise(array, (i) =>
          new Promise((resolve) => setTimeout(resolve, 100 * (10 - i), i)).then(
            updatFinished
          )
        )
        .then(() => {
        ...
        .then(() =>
          finishedOrder.forEach((next, i) => next.should.equal(4 - i))
        );
    });
  });
});
